...
所有阈值在 `navigator.py` 文件头部，直接修改：

| 参数 | 默认值 | 含义 |
|------|--------|------|
| `WAYPOINT_REACH_THRESHOLD` | `25` | 距路标多少像素算"到达" |
| `PATH_DEVIATION_THRESHOLD` | `60` | 偏离路径多少像素触发重规划 |
| `MOVE_DURATION` | `0.15` | 每次按键时长(秒) |
| `TRACK_INTERVAL` | `0.3` | 追踪间隔(秒) |
| `LOOKAHEAD_DIST` | `40` | 向前看多少像素选下一个路标 |
...
导航时自动用**内积最大法**选择最接近路径方向的 8 方向（W/A/S/D 组合）。

### 吸附机制

每步导航前检查当前位置是否在可达区。
如果不在（战斗后被打飞到墙壁里），BFS 搜索最近可达点 → 自动重规划。
